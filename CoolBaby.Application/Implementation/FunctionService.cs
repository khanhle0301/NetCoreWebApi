...
namespace CoolBaby.Application.Implementation
{
    public class FunctionService : IFunctionService
    {
        ...
        public IEnumerable<FunctionViewModel> GetAllWithPermission(string userId)
        {
            using (var sqlConnection = new SqlConnection(_configuration.GetConnectionString("DefaultConnection")))
            {
                ...
                try
                {
                    var query =  sqlConnection.QueryAsync<FunctionViewModel>(
                        "GetAllWithPermission", dynamicParameters, commandType: CommandType.StoredProcedure).Result;

                    var parentIds = query.Select(x => x.ParentId).Distinct();
                    var function = _functionRepository.FindAll(f => parentIds.Contains(f.Id))
